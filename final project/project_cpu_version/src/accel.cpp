...
bool AABB::intersectPoint(const Vec3f point) {
  if (point.x() >= low_bnd.x() && point.y() >= low_bnd.y() && point.z() >= low_bnd.z()
   && point.x() <= upper_bnd.x() && point.y() <= upper_bnd.y() && point.z() <= upper_bnd.z()) {
    return true;
  } else {
    return false;
  }
